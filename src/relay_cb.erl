...
peer_up(_SvcName, Peer, State) ->
    io:fwrite("relay:: start. Peer: ~p~n", [Peer]),
    State.
...
prepare_request(Pkt, _SvcName, Peer, relayed) ->
    io:fwrite("relay:: prepare_request. Peer: ~p~n Pkt: ~p~n", [Peer, Pkt]),
    {send, Pkt}.
...
handle_answer(Pkt, Request, _SvcName, Peer, relayed) ->
    io:fwrite("relay:: handle_request. Peer: ~p~n Pkt: ~p~n Request: ~p~n", [Peer, Pkt, Request]),
    Pkt.
